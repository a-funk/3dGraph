...
| --- | --- | --- |
| Instanced 3D node rendering | `toto-graph/src/render3d.js` | `src/renderers/three-graph.js` |
...
| Generated graph fixtures | New OSS-only API | `src/generators/fixtures.js` |
...
The package now ships a `create3dGraph()` path that imports Three.js and
d3-force-3d directly. A new developer should be able to install the package,
mount a graph into a DOM element, switch generated graph varieties, enter
flight mode, and destroy the viewer without reading Toto source.
